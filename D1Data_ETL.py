...
df = pd.DataFrame()
files = glob.glob(path + r'\D1 Data.xlsx')

for f in files:
    df1 = pd.read_excel(f, sheet_name='D1 Data', skiprows=[2], header = [0,1])
    df1['Filename'] = f
...
df['Time of Arrival'] = df['Time of Arrival'].dt.time
df['Time of Departure'] = df['Time of Departure'].dt.time
...
#Storing Files
df.to_csv(r'\\Computer\e$\QVAPPS\NewProjects\2.DataStore\CSV\D1Table.csv', index=False)
